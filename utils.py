...
def create_price_change_cols():
    ...
    df_price = df_price.sort_values('Date', ascending=True)
...
def merge_price_news(df_price, df_news):
    df_price['Date'] = pd.to_datetime(df_price['Date'])
    df_news['Date'] = pd.to_datetime(df_news['Date'])
    df_merged = pd.merge(left=df_price, right=df_news, left_on='Date', right_on='Date', how='inner')
